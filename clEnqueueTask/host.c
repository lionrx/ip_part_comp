...
FILE *fp;
char fileName[] = "C:/Users/Shermit/Desktop/USMC/Year_3/IP/IP-particle-fluid-sim-master_mine/clEnqueueTask/propagate.cl";		// propagate.cl file location
char *source_str;
size_t source_size;
...
int main() {
	...
	for (float time = 0; time < tfinal; time += 2 * tstep) {
		...
		for (particle = 0; particle < NUMPART; particle++) {
			...
			ret = clSetKernelArg(kernel, 25, sizeof(int), &particle);
